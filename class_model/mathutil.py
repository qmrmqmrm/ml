...
def onehot(xs, cnt):
    return np.eye(cnt)[np.array(xs).astype(int)]            # 단위행렬 생성 cnt크기만큼 int타입인 xs에 맞게
...
def load_image_pixels(imagepath, resolution, input_shape):
    img = Image.open(imagepath)         # 이미지 열기
    resized = img.resize(resolution)    # 이미지 크기 조절 resoultion=100,100
    # print(np.array(resized).)
    # print(np.array(resized).reshape(input_shape).shape)
    return np.array(resized).reshape(input_shape) # 이미지를 1차원 행렬로 넘겨준다. (100,100,3) -> (30000,)
...
def list_dir(path):
    filenames = os.listdir(path)            #filenames = daisy, dandelion, rose, sunflower, tulip
    filenames.sort()                        # 이름 순으로 정렬
    return filenames